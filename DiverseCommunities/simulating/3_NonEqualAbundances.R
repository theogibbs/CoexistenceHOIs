########################################################
# Non-equal abundances variances simulations for
# panel (B) of Fig. 3
########################################################
...
# choosing the variances in the target abundances
...
for(cur_row in 1:nrow(in_pars)) {
  ...
  # looping over the different variances in abundances
  for(cur_noise in abundance_noise) {
    
    # sampling the target abundances and constrained HOIs
    ...
    feas_data <- data.frame(Stable = cur_stable, Noise = cur_noise, Type = "Carrying Capacity")
