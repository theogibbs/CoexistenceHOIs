########################################################
# Non-equal abundance simulations for
# panel (C) of Fig. 3
########################################################
...
num_repl <- 50 # choosing the number of replicates
...
for(cur_row in 1:nrow(in_pars)) {
  ...
  # looping over the different SADs and variances in the abundances
  for(cur_dist in in_dists) {
    for(cur_val in sd_vals) {
      
      # sampling the target abundances
      cur_pars$r <- rep(1, times = cur_pars$S)
      target_abd <- GetTargetAbd(pars = cur_pars, choice = cur_dist,
                                 value = ifelse(cur_dist == "Carrying Capacities",
                                                0.5 * cur_val, cur_val))
      # choosing the growth rates so the target abundances are the carrying capacities
      cur_pars$r <- target_abd
      # sampling the constrained HOIs with non-equal abundances
      ...
      # recording stability and feasibility
...
filename <- "RealisticSADsCarryingCapacitiesHigherVariance"
