########################################################
# Non-equal abundance simulations for SI Fig. P
########################################################
...
sd_vals <- seq(0.25, 1, length.out = 5)
...
for(cur_row in 1:nrow(in_pars)) {
  ...
  for(cur_dist in in_dists) {
    for(cur_val in sd_vals) {
      ...
      #cur_pars$B <- GetAbdVarianceFeasibleB(target_abd, cur_pars)
      ...
      feas_data <- data.frame(Stable = cur_stable, SAD = cur_dist, SD = cur_val,
                              Real = re_eigs, Imaginary = im_eigs, MeanB = mean_B, MeanJ = mean_J)
...
filename <- "VaryingTargetAbundances"
