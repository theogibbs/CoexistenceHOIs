########################################################
# Diverse community simulations minimizing the
# eigenvalues as a function of the target abundances
########################################################

# Loading dependencies

source("./Functions.R")
source("./DiverseCommunities/DiverseCommunityFunctions.R")

# Simulating script

S <- 5
mu_R <- 1
sigma_R <- 0
mu_D <- 1
sigma_D <- 0
mu_A <- 0
sigma_A <- seq(0, 1, length.out = 5)
rho_A <- 0
mu_B <- 0
sigma_B <- 0
intra <- "None"
self_reg <- "Quadratic"
scaling <- FALSE
dist_B <- "Normal"

in_pars <- crossing(S = S,
                    MuR = mu_R,
                    SigmaR = sigma_R,
                    MuD = mu_D,
                    SigmaD = sigma_D,
                    MuA = mu_A,
                    SigmaA = sigma_A,
                    RhoA = rho_A,
                    MuB = mu_B,
                    SigmaB = sigma_B,
                    Intra = intra,
                    SelfReg = self_reg,
                    scaling = scaling,
                    DistB = dist_B)

out_data <- data.frame()

num_repl <- 20
in_pars <- bind_rows(replicate(num_repl, in_pars, simplify = FALSE))
in_pars$ParsID <- runif(nrow(in_pars))

out_data <- data.frame()

start_time <- Sys.time()

for(cur_row in 1:nrow(in_pars)) {
  cur_stats <- in_pars[cur_row,]
  
  cur_pars <- BuildPars(cur_stats)
  
  cur_min <- optimize(f = GetMinEig, interval = c(0, 10), cur_pars)
  
  hoi_eig <- cur_min$objective
  hoi_stable <- (hoi_eig < 0)
  
  min_data <- data.frame(TargetAbd = cur_min$minimum,
                         Eigenvalue = hoi_eig,
                         Stable = hoi_stable,
                         Type = "Minimized")
  
  target_abd <- rep(GetTargetAbd(cur_pars), times = cur_pars$S)
  const_B <- GetFeasibleB(target_abd, cur_pars)
  cur_pars$B <- const_B
  
  hoi_eig <- GetEig(BuildJacobian(target_abd, cur_pars))
  hoi_stable <- (hoi_eig < 0)
  
  car_data <- data.frame(TargetAbd = unique(target_abd),
                         Eigenvalue = hoi_eig,
                         Stable = hoi_stable,
                         Type = "Carrying Capacity")
  
  cur_data <- rbind(min_data, car_data)
  cur_data <- cbind(cur_stats, cur_data)
  out_data <- rbind(out_data, cur_data)
}

print(Sys.time() - start_time)

# writing out the data
filename <- "DiverseMinimaStability"
cur_file <- paste0("./simdata/", filename, ".csv")
write_csv2(out_data, file = cur_file)


